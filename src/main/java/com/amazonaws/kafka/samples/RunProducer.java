...
public class RunProducer implements Callable<String>{
    ...
    private final Producer<java.lang.String, WorkspaceEvent> kafkaProducer;
    private boolean cancel = false;
    private static AtomicInteger userIDMax = new AtomicInteger(1000);
    private Random rand = new Random();

    RunProducer(Producer<java.lang.String, WorkspaceEvent> kafkaProducer) {
    ...
    private void runProducer() throws Exception {
        ...
        Integer userIDLimit;
        UtilizationEvents producer = new UtilizationEvents();

        logger.info("Starting producer in thread {} \n", Thread.currentThread().getName());

        while (userCount < KafkaEventstreamClient.numberOfUsers) {
            ...
            if (!cancel) {
                ...
                if (KafkaEventstreamClient.counter.get() == 0L) {
            ...
            userCount += 1;
            if (!(KafkaEventstreamClient.noDelay)) {
                Util.Sleep(500);
            }
        }
