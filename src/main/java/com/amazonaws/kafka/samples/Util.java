...
class Util {
    ...
    static void eventWriter(String event, String filesystemLocation, boolean append, String eventType) throws IOException {
        ...
        if (eventType.equalsIgnoreCase("bookmark")){
            ...
            if (counter.incrementAndGet() == KafkaEventstreamClient.numThreads){
    ...
    static void setGlobalSeqNo(String filesystemLocation) throws IOException {
        try {
            ...
            KafkaEventstreamClient.startingGlobalSeqNo = Long.parseLong(br.readLine());
            KafkaEventstreamClient.counter.set(KafkaEventstreamClient.startingGlobalSeqNo);
            logger.info("Starting Global Seq No: {} \n", KafkaEventstreamClient.startingGlobalSeqNo + 1);
