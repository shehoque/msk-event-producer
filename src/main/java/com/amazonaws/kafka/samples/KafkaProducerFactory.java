...
class KafkaProducerFactory {
    ...
    private static final Logger logger = LogManager.getLogger(KafkaEventstreamClient.class);

    private static String getSaslScramString() {
        String secretNamePrefix = "AmazonMSK_";
        String secret = Secrets.getSecret(secretNamePrefix + KafkaEventstreamClient.saslscramUser, Secrets.getSecretsManagerClient(KafkaEventstreamClient.region));
        ObjectMapper objectMapper = new ObjectMapper();
        JsonNode jsonNode;
        try {
            jsonNode = objectMapper.readTree(secret);
        } catch (IOException e) {
            logger.error("Error reading returned secret for user {} \n", KafkaEventstreamClient.saslscramUser);
            logger.error(Util.stackTrace(e));
            throw new RuntimeException(String.format("Error reading returned secret for user %s \n", KafkaEventstreamClient.saslscramUser));
        }
        String password = jsonNode.get("password").asText();
        return "org.apache.kafka.common.security.scram.ScramLoginModule required username=" + KafkaEventstreamClient.saslscramUser + " password=" + password + ";";
    }
    ...
    Producer<String, WorkspaceEvent> createProducer() throws Exception {
        ...
        try (FileInputStream file = new FileInputStream(propertiesFilePath)) {
            ...
            if (glueSchemaRegistry){
                ...
                producerProps.setProperty(AWSSchemaRegistryConstants.AWS_REGION, KafkaEventstreamClient.gsrRegion);
                producerProps.setProperty(AWSSchemaRegistryConstants.AVRO_RECORD_TYPE, AvroRecordType.SPECIFIC_RECORD.getName());
                if (KafkaEventstreamClient.gsrAutoRegistration)
                    producerProps.setProperty(AWSSchemaRegistryConstants.SCHEMA_AUTO_REGISTRATION_SETTING, "true");
                if (KafkaEventstreamClient.gsrRegistryName != null)
                    producerProps.setProperty(AWSSchemaRegistryConstants.REGISTRY_NAME, KafkaEventstreamClient.gsrRegistryName);
                if (KafkaEventstreamClient.gsrSchemaName != null)
                    producerProps.setProperty(AWSSchemaRegistryConstants.SCHEMA_NAME, KafkaEventstreamClient.gsrSchemaName);
                if (KafkaEventstreamClient.gsrSchemaDescription != null)
                    producerProps.setProperty(AWSSchemaRegistryConstants.DESCRIPTION, KafkaEventstreamClient.gsrSchemaDescription);
                if (KafkaEventstreamClient.gsrCompatibilitySetting != null)
                    producerProps.setProperty(AWSSchemaRegistryConstants.COMPATIBILITY_SETTING, Compatibility.fromValue(KafkaEventstreamClient.gsrCompatibilitySetting).toString());
            } else {
